...
uint8_t lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW; // Set when the LCD needs to draw, decrements after every draw. Set to 2 in LCD routines so the LCD gets at least 1 full redraw (first redraw is partial)
  ...
  static void lcd_setting_zoffset_menu();//20170202
...
void kill_screen(const char* lcd_msg) {
  ...
  static void lcd_main_menu() {
    ...
    MENU_ITEM(submenu, MSG_SETTING_ZOFFSET , lcd_setting_zoffset_menu);//20170202
  ...
  static void lcd_prepare_menu() {
    ...
    // Set Home Offsets
    //
    //  MENU_ITEM(function, MSG_SET_HOME_OFFSETS, lcd_set_home_offsets);//20170202
   ...
   */
   static void lcd_setting_zoffset_menu()
   {
    ...
    MENU_ITEM(gcode, MSG_LEVEL_BED,axis_homed[X_AXIS] && axis_homed[Y_AXIS] ? PSTR("G29") : PSTR("G28\nG29") );

    MENU_ITEM(function, MSG_GOTO_ZERO, deploy_z_zero);

    MENU_ITEM_EDIT_CALLBACK(float52,  MSG_ZPROBE_ZOFFSET, &zprobe_zoffset, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX, Config_StoreSettings);
